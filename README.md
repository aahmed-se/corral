...
For the companion extension that edits Chrome bookmarks directly, see [Bookmark OS](bookmark-os/README.md). The [review and remaining priorities](docs/improvement-review.md) document this improvement pass.

## What it does

- **Add and edit**: save individual links, edit titles/URLs/folders, and undo edits. Use the row pencil, right-click menu, or F2. Manual saves validate URLs and retain existing creation/import dates.
...
- **Non-destructive imports** from Chrome (Chrome's tree is never modified), Chrome/Firefox/Netscape HTML exports, or Corral JSON backups. Re-importing keeps the current Corral rows and folder organization intact, skips normalized URLs already present, and adds only new entries. Export both file formats back out. HTML exports retain true folder nesting and empty folders; JSON backups also retain empty folders and import timestamps.
...
- **Duplicate cleanup**: one toolbar button scans the library by normalized URL (tracking parameters, fragments, and case ignored), keeps the oldest copy of each link, and removes the rest — with a confirm step and Undo for up to 50,000 removals. Larger deletions require a separate confirmation that explains the recovery limit.
...
- **Narrow screens and keyboard access**: collapsible folders, clickable breadcrumbs, visible edit/open actions, focus-managed dialogs, and `/` or Cmd/Ctrl+K to focus search.
...
- `src/lib/worker.ts` — one streamed pass builds search shards + view ordering with per-phase progress. Mutations run as serialized ops that patch the served index immediately and then save just the patch (delta shard rows, the view arrays, tombstones, and a stats row whose dirty token vouches for the op); imports and manual add/edit operations rescan. When tombstones or delta shards pile up, a compaction rebuild runs quietly behind the current index. Imports, exports, and parsing (a DOMParser-free tokenizer) all execute here.
